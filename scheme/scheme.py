...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        """if check_procedure(scheme_symbolp(first)):
            print (6)"""
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError
        while formals != nil: #assigning the new values
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        operators = operands.map(lambda x: scheme_eval(x, env))
        apply_functions = scheme_apply(self, operators, env )
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        #create a child frame for the call
        # BEGIN PROBLEM 12
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        applied_functions = scheme_apply(self, operands, env)
        return scheme_eval(complete_eval(applied_functions), env)
        # END Problem 21
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        expdef = expressions.second.first
        env.define(target, scheme_eval(expdef, env))
        return  target
        "*** YOUR CODE HERE ***"
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        #assign user-defined functions to the lambda class
        name = target.first
...
def do_and_form(expressions, env):
    ...
    if expressions.second==nil:
        predicate = scheme_eval(expressions.first, env, True)
    else:
        predicate = scheme_eval(expressions.first, env)
    if scheme_falsep(predicate):
        return False
    else:
...
def make_let_frame(bindings, env):
    ...
    symbols,vals, bindings = Pair(bindings.first.first, nil), Pair(scheme_eval(bindings.first.second.first, env), nil), bindings.second
    while bindings:
        check_form(bindings.first, 2, 2)
        check_form(bindings.first, 2, 2)
        symbols, vals, bindings = Pair(bindings.first.first, symbols), Pair(scheme_eval(bindings.first.second.first, env), vals), bindings.second
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        name = target.first
        function = MacroProcedure(target.second, expressions.second, env)
        env.define(name, function)
        return name
        # END PROBLEM 10
    else:
